...
const notify = (message) => {
  ...
  const ps = [
    ...
    '$x = [xml]$t.GetXml()',
    `$x.GetElementsByTagName('text')[0].AppendChild($x.CreateTextNode('Codex')) | Out-Null`,
    ...
    `[Windows.UI.Notifications.ToastNotificationManager]::CreateToastNotifier('Codex').Show($n)`,
  ].join('; ');
